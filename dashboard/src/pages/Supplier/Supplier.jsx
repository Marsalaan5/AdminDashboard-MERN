...
//   const handleSubmitAdd = async () => {
//   // Validate the data to make sure no required field is missing or null
//   if (!newSupplierData.name || !newSupplierData.company || !newSupplierData.address) {
//     alert("Name, company, and address are required!");
//     return;
//   }
...
const handleSubmitAdd = async () => {
  ...
  const supplierWithKey = {
    ...newSupplierData,
    key: `${newSupplierData.company}_${Date.now()}` // e.g., "AcmeCorp_1715529812345"
  };

  try {
    const response = await axios.post('http://localhost:5000/api/suppliers', supplierWithKey);
    setSuppliers((prevSupplier) => [...prevSupplier, response.data]);
    setShowAddModal(false);
  } catch (error) {
    console.error("Error adding supplier!", error);
...
<div className="container-fluid">
        <div className="d-md-flex d-block align-items-center justify-content-between page-header-breadcrumb">
          ...
          <div className="header_svg d-flex">
           {user?.role==="admin" && ( <Button variant="contained" color="primary"  startIcon={<Add />} onClick={handleAddSupplier}>
              Add Supplier
            </Button>)}
          </div>
        </div>
